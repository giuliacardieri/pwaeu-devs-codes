...
const getDB = function getDB() {
  if (localStorage.getItem('pareionde')){
    console.log(localStorage.getItem('pareionde'))
...
const loadDesignListaTemplate = function loadDesignListaTemplate() {
  ...
  html = template(model)
  $('.main__meudesign-wrapper').removeAttr('hidden').html(html)
  $('.accordion__lista-wrapper').html(html)
}

/* carrega/recarrega o template do formulário de adicionar séries  */
